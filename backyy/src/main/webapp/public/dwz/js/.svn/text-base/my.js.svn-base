...
*/
 ...
 $(".linkage").change(function(){
    ...
    var linkage_format=$th.attr("linkage_format");
    var linkage_keep=obj.attr("linkage_keep")?obj.attr("linkage_keep"):0;       
	var selectname=$th.attr("selectname");
    $.ajax({ 
        url:linkage_href+nb,
        global:false,
        dataType:"json",
        success:function(data){          
            ...
            if(!jQuery.isEmptyObject(data)){//判断Data是否为空值
               /* var current_format=linkage_format;                           
                for(var s in data){
                    current_format=linkage_format;   
                    for(var f in data[s]){
		        ...
		        obj.append(str);
		        obj.val(obj.attr("selectvl"));
		        var selectname=obj.attr("selectname");
		        if(selectname!=""){
		        	var id=obj.attr("id");		        	
		        	$("#"+id+" option").each(function(i,n){
		        		if($(n).text()==selectname)
		        		{
  ...
  //下拉列表框根据文本选中
    $("select[selectname]",$p).each(function(){
 ...
 */
	 ...
	 * 客户信息弹出层
	 */
	$('.ajax_ptip_user', $p).each(function() {
		var url = $(this).attr("ptiprel");
		$(this).poshytip( {
			alignTo : "target",
			alignX : "right",
			alignY : "center",
...
function treeStyle($th,treeid,tree_style,tree_reload){
    var tree_href=$th.attr("tree_href")+"&currentTime="+new Date();                                                //ajax获取数据所需的连接地址
    $.ajax({
        url:tree_href,
        global:false,
        dataType:"json",
