...
function initUI(_box){
    ...
    $(".permission",$p).each(function(){
    	...
    	var carteId=$("input[name='carte_id']",$p).val();
    	var allHref=$("#allHref").val();//首页里，该用户的所有权限文本框值 形成,user/add,user/delete,类似的
    	if(allHref!= undefined && allHref!=""){
	    	if(allHref.indexOf(","+href+"-"+carteId)==-1 && allHref.indexOf(",i am leader")==-1){   //不存在  管理员的话加上这句代码即可 现处于测试阶段 && allHref.indexOf(",i am leader")==-1
	    		$btn.hide();
	    	}
    	}
    });
