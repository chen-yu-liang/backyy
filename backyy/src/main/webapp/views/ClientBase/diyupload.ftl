      ...
      <#list basepic as basepic>
			...
			<script type="text/javascript">
				$("#"+"${(basepic['INFO_EN'])!}").diyUpload({
							url : '${(projectName)!}/?spm=${(urlEncrption('ClientBase/uploadPic'))!}&client_no=${(client_no)}&class_en=${(basepic['INFO_EN'])!}&acount_type=0',
							success : function(data) {
					  			if(data.pic=="0000"){
					  			 ...
					  			 //$.magnificPopup.close();
					  			   //刷新页面
					  			   navTab.reloadFlag("ClientBase_findpictrue");
					  			}else{
