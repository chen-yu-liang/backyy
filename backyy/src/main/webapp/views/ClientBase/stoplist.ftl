...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="delete permission" href="${(projectName)!}/?spm=${(urlEncrption('/${(action)!}/update'))!}&id={id}&status=2" target="ajaxTodo" rel="ClientBase_update" title="确定要停启用【{cn}】吗?"><span>启用</span></a></li>
		...
		<tbody>
			<#list p.results as list>
				<tr target="id" rel="${(list.CLIENT_NO)!}">
					...
					<td >${(list.PERSON)!}</td>
					<td >${(list.MOBILE)!}</td>
					<td ><#if list.STATUS==-1>停用</#if>[${(list.STATUS)!}]</td>
				</tr>
			</#list>
		</tbody>
	</table>
