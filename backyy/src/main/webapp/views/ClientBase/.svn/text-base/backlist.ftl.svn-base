...
<div class="pageContent">
	<div class="panelBar">
		...
		-->
			<li><a class="edit permission" href="${(projectName)!}/?spm=${(urlEncrption('/${(action)!}/add'))!}&id={id}" target="dialog"  width="900" height="500" rel="ClientBase_add" title="修改-{cn}"><span>修改</span></a></li>
		...
		<tbody>
			<#list p.results as list>
				<tr target="id" rel="${(list.CLIENT_NO)!}">
					...
					<td >${(list.PERSON)!}</td>
					<td >${(list.MOBILE)!}</td>
					<td ><#if list.STATUS==-2>打回</#if>[${(list.STATUS)!}]</td>
				</tr>
			</#list>
		</tbody>
	</table>
