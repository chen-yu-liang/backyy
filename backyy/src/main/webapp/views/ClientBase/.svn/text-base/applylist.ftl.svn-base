...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add permission" href="${(projectName)!}/?spm=${(urlEncrption('/${(action)!}/add'))!}" target="dialog" rel="ClientBase_add" width="900" height="520" ><span>添加商户</span></a></li>
			<li><a class="edit permission" href="${(projectName)!}/?spm=${(urlEncrption('/${(action)!}/add'))!}&id={id}" target="dialog"  width="900" height="500" rel="ClientBase_add" title="修改-{cn}"><span>修改商户</span></a></li>
			<li><a class="delete permission" href="${(projectName)!}/?spm=${(urlEncrption('/${(action)!}/delete'))!}&id={id}" target="ajaxTodo" title="确定要删除【{cn}】吗?"><span>删除商户</span></a></li>
			<li><a class="edit permission" href="${(projectName)!}/?spm=${(urlEncrption('/${(action)!}/update'))!}&id={id}&status=1" target="ajaxTodo" rel="ClientBase_applylist" title="确定要送审【{cn}】吗?"><span>送审</span></a></li>
			<li><a class="edit permission" href="${(projectName)!}/?spm=${(urlEncrption('ClientBase/findpictrue'))!}&client_no={cm}&client_name={cn}&status=0" target="navTab" rel="ClientBase_findpictrue"><span>图片信息</span></a></li>
		</ul>
	</div>
