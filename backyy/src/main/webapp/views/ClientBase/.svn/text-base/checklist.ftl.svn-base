...
<div class="pageContent">
	<div class="panelBar">
		...
		-->
			...
			<li><a class="delete permission" href="${(projectName)!}/?spm=${(urlEncrption('/${(action)!}/updateCheck'))!}&id={id}&status=2" target="ajaxTodo" rel="ClientBase_checklist" title="确定要审核【{cn}】吗?"><span>审核</span></a></li>
			<li><a class="delete permission" href="${(projectName)!}/?spm=${(urlEncrption('/${(action)!}/update'))!}&id={id}&status=-2" target="ajaxTodo" rel="ClientBase_checklist" title="确定要打回【{cn}】吗?"><span>打回</span></a></li>
			<li><a class="edit permission" href="${(projectName)!}/?spm=${(urlEncrption('/ClientProduct/list'))!}&client_no={id}" target="dialog"  width="900" height="500" rel="ClientProduct_list" title="开通-{cn}的产品"><span>开通产品</span></a></li>
		</ul>
	</div>
