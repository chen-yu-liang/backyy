...
<div class="pageContent">
		        ...
		        <table style="width:100%;text-align: center; margin-top:15px;" class="aa">
						<tr>
							<td style="text-align: center;">
							   <img alt="${(files['FILES_NAME'])!}--点击查看大图（旋转照片）" src="${(files['files_address'])!}" 
							    class="artZoom imgsss" data-artZoom-show="${(files['files_address'])!}"  data-artZoom-source="${(files['files_address'])!}" />
						    </td>
						</tr>
						<#if status??>
						<tr>
							<td style="height: 40px;">
							    <div class="picDiv">${(files['FILES_NAME'])!}&nbsp;&nbsp;&nbsp; 
							         <a href="${(projectName)!}/?spm=${(urlEncrption('Agent/deletePic'))!}&id=${(files['FILES_ID'])!}&pic_name=${(files['FILES_NAME'])!}" style="color:blue;" target="ajaxTodo" title="确认删除吗？">(删除)</a>
						        </div>
						    </td>
						</tr>
						</#if>
            ...
            <#if files['files_type']=="other">
		        ...
		        <table style="width:100%;text-align: center;  margin-top:15px;" class="dd">
						...
						<tr>
							<td style="height: 40px;">
							    <div class="picDiv">${(files['FILES_NAME'])!}&nbsp;&nbsp;&nbsp; 
							         <a class="delete permission" href="${(projectName)!}/?spm=${urlEncrption("/Agent/deletePic")}&id=${(files['FILES_ID'])!}&pic_name=${(files['FILES_NAME'])!}" style="color:blue;" target="ajaxTodo" rel="Agent_deletePic" title="确认删除吗？">(删除)</a>
						        </div>
						    </td>
						</tr>
						</#if>
   ...
   <div class="formBar">
			<ul>
   			<#if status??>
			     <li>
					<a class="buttonActive" href="${(projectName)!}/?spm=${(urlEncrption('Agent/diyupload'))!}&client_no=${client_no}" rel="Agent_diyupload" target="dialog" width="890" height="560">
					   <span>上传商户图片资料</span>
					</a>
				 </li>
   			</#if>
