...
<script>
	...
	$(function(){
		...
		var flag = false;
		var id = "${(map['ID'])!}";
		$("#btnManage").attr("href","${(projectName)!}/?spm=${(urlEncrption('Permission/list'))!}&carte_id=${(map['ID'])!}");
		$("#dataPermission").attr("href","${(projectName)!}/?spm=${(urlEncrption('DataPermission/list'))!}&carte_id=${(map['ID'])!}");
		if(parentroot!=0){
			...
			if(defaultOpen=="1"){
				$box = $("#btnPageForm");
				url = "${(projectName)!}/?spm=${(urlEncrption('Permission/list'))!}";
				flag = true;
			}
			if(defaultOpen=="2"){
				$box = $("#dataPageForm");
				url = "${(projectName)!}/?spm=${(urlEncrption('DataPermission/list'))!}";
				flag = true;
			}
			if(flag==true){
...
<div class="tabs tabsCurrentIndex" currentIndex = "0" style="margin-top:-9px">
			...
			<div class="tabsHeaderContent">
				<ul>
					...
					<#if (map["PARENTROOT"])?default(0)?number !=0>
						<li><a href="${(projectName)!}/?spm=${(urlEncrption('Permission/list'))!}&carte_id=${(map['ID'])!}" class="j-ajax" id = "btnManage"><span>按钮\行为管理</span></a></li>
						<!--<li><a href="${(projectName)!}/?spm=${(urlEncrption('DataPermission/list'))!}&carte_id=${(map['ID'])!}" class="j-ajax" id = "dataPermission"><span>数据权限管理</span></a></li>-->
					</#if>
				</ul>
			</div>
		</div>
		<div class="tabsContent" >
			<div class="pageContent">
				<form method="post" action="${(projectName)!}/?spm=${(urlEncrption('BackCarte/save'))!}" class="pageForm required-validate" onsubmit="return validateCallback(this, navTabAjaxDone);">
				...
				<input name="belong_terrace" value="${(platSign)!'0'}" type = "hidden"/>
				<div class="pageFormContent" layoutH="175" style="border: 1px solid #999999;background-color:#FFFFFF">
					...
					<p>
						...
						<input type="text" class="mtree required" name="organ_nbname" value="${(map['PARENTCARTE'])!'主菜单'}" ztree_remove="${(map['PARENT_ID'])!}"
						 treeH="200" treeW="185" size="27" ztree_open="false" ztree_top="主菜单" tree_href="${(projectName)!}/?spm=${(urlEncrption('BackCarte/publicMenu'))!}&platSign=${(platSign)!'0'}"/>
					...
					<p style="height:auto;width:100%">
						...
						<textarea name="carte_intro" style="width:800px;height:200px" tools="simple">${(map["CARTE_INTRO"])!}</textarea>
					</p>
					<p>
