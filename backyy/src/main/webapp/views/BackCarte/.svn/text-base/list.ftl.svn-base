<script>
	//清除缓存
	function clearCache(){
		$.post("${(projectName)!}/?spm=${(urlEncrption('Cache/clearCache'))!}",function (json){
			DWZ.ajaxDone(json);
	...
	function getMenuName(){
		...
		alertMsg.confirm("确认删除【"+menuName+"】菜单么？",{
			okCall:function(){
				$.post("${(projectName)!}/?spm=${(urlEncrption('BackCarte/delete'))!}&id="+id,function (json){
					DWZ.ajaxDone(json);
					if(json.navTabId){
	...
	//判断选择平台
	function reloadPlat(vl){
		var $tabid = $("li[tabid='BackCarte_list']");
		var url = $tabid.attr("url");
	...
	<div class="formBar">
		<ul style="float:left;height:30px;">
		    <li style="margin-left:20px;">
				所属平台：
					<select id = "belongTerrace" onchange="reloadPlat(this.value)" selectvl = "${(platSign)!}">
