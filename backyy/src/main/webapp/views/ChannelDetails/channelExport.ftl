...
$(function(){
	$("#file").change(function(){
		...
		if(suffix != '.xls'&&suffix != '.xlsx'){
			$("#tishi").css({"color":"red","font-size":"12px"}).text("请重新选择.xls文件！");
			$("#noChoice").css("display","none");
		}else{
...
function getFilePath(){  
 ...
 <div class="pageContent" style="overflow:hidden;">
  <form method="post" action="${(projectName)!}/?spm=${(urlEncrption('/${(action)!}/saveChannel'))!}" enctype="multipart/form-data" class="pageForm required-validate" onsubmit="return iframeCallback(this,dialogAjaxDone);" novalidate="novalidate" target="callbackframe">
		<div class="pageFormContent" layouth="55" style="height: 89px; overflow: auto;background-color:#ecf2f4;">
			<P>
			...
			<label style="color:green;">选择Excel：</label>
