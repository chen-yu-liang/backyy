...
<div class="pageContent">
	...
	<table class="table" width="98%" layoutH="109">
		<thead>
			<tr>
				<th width="10%" orderField="s0_rate_min"></th>
				<th width="10%" orderField="d1_rate_max"></th>
				<th width="10%" orderField="agent_no"></th>
				<th width="10%" orderField="d1_rate_min"></th>
				<th width="10%" orderField="s0_rate_max"></th>
			</tr>
		</thead>
		<tbody>
			<#list p.results as list>
				<tr target="id" rel="${(list.ID)!}">
					<td >${(list.S0_RATE_MIN)!}</td>
					<td >${(list.D1_RATE_MAX)!}</td>
					<td >${(list.AGENT_NO)!}</td>
					<td >${(list.D1_RATE_MIN)!}</td>
					<td >${(list.S0_RATE_MAX)!}</td>
				</tr>
			</#list>
		</tbody>
	</table>
