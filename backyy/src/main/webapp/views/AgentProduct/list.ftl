<form id="pagerForm" action="${(projectName)!}/?spm=${(urlEncrption('/AgentProduct/list'))!}" method="post">
...
<div class="pageHeader">
	<form onsubmit="return dialogSearch(this);" action="${(projectName)!}/?spm=${(urlEncrption('/AgentProduct/list'))!}" method="post">
		<div class="searchBar">
			<table class="searchContent">
				<tr>
						<td>
							...
							<input type="hidden" name="agent_no" value="${(p.params.agent_no)!''}" size=15/>
						<td>
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add permission" href="${(projectName)!}/?spm=${(urlEncrption('/${(action)!}/add'))!}&id=${(p.params.agent_no)!''}" target="dialog" rel="AgentProduct_add" width="450"><span>添加</span></a></li>
			<li><a class="edit permission" href="${(projectName)!}/?spm=${(urlEncrption('/${(action)!}/add'))!}&id={id}&product_no={pn}&clear_type={cn}" target="dialog"  width="450" rel="AgentProduct_add" title="修改-{class_cn}"><span>修改</span></a></li>
			<li><a class="delete permission" href="${(projectName)!}/?spm=${(urlEncrption('/${(action)!}/delete'))!}&id={id}&product_no={pn}&clear_type={cn}" target="ajaxTodo" title="确定要删除【{class_cn}】吗?" callback = "ourAjaxDone"><span>删除</span></a></li>
		</ul>
	</div>
	<table class="table" width="98%" layoutH="109">
		<thead>
			<tr>
				...
				<th width="10%" orderField="product_no">产品代号</th>
		...
		<tbody>
			<#list p.results as list>
				<tr target="id" rel="${(list.AGENT_NO)!}">
					...
					<td target="class_cn" rel="${(list.AGENT_NO)!}">${(list.PRO_NAME)!}</td>
